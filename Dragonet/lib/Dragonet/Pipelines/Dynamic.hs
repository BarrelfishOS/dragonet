...
run ctx pconn pcreate plg = do
    ...
    pinit pli = do
        ...
        putStrLn $ "pipeline: " ++ lbl ++ " inDel:" ++ (show inDel)
        putStrLn $ "pipeline: " ++ lbl ++ " inAdd:" ++ (show inAdd)
        putStrLn $ "pipeline: " ++ lbl ++ " outDel:" ++ (show outDel)
        putStrLn $ "pipeline: " ++ lbl ++ " outAdd:" ++ (show outAdd)
