...
eventHandler sstv ch (PLA.EvSocketSpan oldsid) = do
    (sid,ss) <- STM.atomically $ do
        ...
        let Just aid = M.lookup ch $ ssAppChans ss
            ...
            eid = sdEpId oldsd -- ^ get endpointID from old socket
            Just ep = M.lookup eid $ ssEndpoints ss -- ^ Find the old endpoint
            -- Create new socket with appID and old endpointID
            sd = SocketDesc { sdAppId = aid, sdEpId = eid }
            -- Add this socket into the list of sockets associated with the endpoint
            ep' = ep { edSockets = edSockets ep ++ [sid] }
            -- Update the stackState with
            ss' = ss {
                ssNextSocketId = sid + 1, -- incremented SID
                ssSockets = M.insert sid sd $ ssSockets ss, -- new socket inserted
                -- inserting updated EP back at same index where it was before
                --      So, this time it will have additional socket
                ssEndpoints = M.insert eid ep' $ ssEndpoints ss
            }
        STM.writeTVar sstv $ ss'
        return (sid,ss')
