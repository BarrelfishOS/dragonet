...
node_out_t do_pg__RxL3IPv4ValidHeaderLength(
        struct ctx_RxL3IPv4ValidHeaderLength *context, struct state *state,
        struct input **in)
{
...
node_out_t do_pg__RxL3IPv4ValidReassembly(
        struct ctx_RxL3IPv4ValidReassembly *context, struct state *state,
        struct input **in)
{
    dprint("%s:%s:%d \n", __FILE__, __func__, __LINE__);
    // P_true, P_false
...
node_out_t do_pg__RxL3IPv4ValidVersion(struct ctx_RxL3IPv4ValidVersion *context,
...
node_out_t do_pg__RxL3IPv4ValidLength(struct ctx_RxL3IPv4ValidLength *context,
        struct state *state, struct input **in)
{
    dprint("%s:%s:%d \n", __FILE__, __func__, __LINE__);
    // P_true, P_false
...
node_out_t do_pg__RxL3IPv4ValidTTL(struct ctx_RxL3IPv4ValidTTL *context,
        struct state *state, struct input **in)
{
    dprint("%s:%s:%d \n", __FILE__, __func__, __LINE__);
    // P_true, P_false
...
node_out_t do_pg__RxL3IPv4ValidChecksum(
        struct ctx_RxL3IPv4ValidChecksum *context, struct state *state,
        struct input **in)
{
    dprint("%s:%s:%d \n", __FILE__, __func__, __LINE__);
    // P_true, P_false
    return PORT_BOOL(ipv4_checksum(*in, ipv4_hdroff(*in),
                ipv4_hdrlen(*in)) == 0);
}

node_out_t do_pg__RxL3IPv4ValidLocalIP(struct ctx_RxL3IPv4ValidLocalIP *context,
        struct state *state, struct input **in)
{
    dprint("%s:%s:%d \n", __FILE__, __func__, __LINE__);
...
node_out_t do_pg__RxL3IPv4Classify(struct ctx_RxL3IPv4Classify *context,
        struct state *state, struct input **in)
{
    dprint("%s:%s:%d \n", __FILE__, __func__, __LINE__);
...
node_out_t do_pg__TxL3IPv4AllocateHeader(
        struct ctx_TxL3IPv4AllocateHeader *context, struct state *state,
        struct input **in)
{
    dprint("%s:%s:%d \n", __FILE__, __func__, __LINE__);
...
node_out_t do_pg__TxL3IPv4FillHeader(struct ctx_TxL3IPv4FillHeader *context,
        struct state *state, struct input **in)
{
    dprint("%s:%s:%d \n", __FILE__, __func__, __LINE__);
...
node_out_t do_pg__TxL3IPv4Routing(struct ctx_TxL3IPv4Routing *context,
        struct state *state, struct input **in)
{
    dprint("%s:%s:%d \n", __FILE__, __func__, __LINE__);
    // P_true, P_false
